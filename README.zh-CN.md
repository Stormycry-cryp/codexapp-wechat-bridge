...
在看过 `项目列表` 或 `线程列表` 后，直接回复纯数字会复用最近一次展示的列表上下文并直接切换。审批态下的 `1` / `2` 仍然优先表示同意 / 拒绝。

bridge 会为每个项目保存一个活动线程，所以切换项目时会恢复那个项目最近的活动线程（如果存在）。
...
| `/threads`, `/thread`, `线程`, `线程列表` | 查看最近线程；随后可直接回复数字切换 |
...
- `projects.json`：手动固定的项目快捷方式。
- `bridge-state.json`：当前项目、每个项目的活动线程 id，以及最近一次列表上下文（用于纯数字切换）。
